#!/usr/bin/env bash

set -u
#set -x
set -v

SETUP_DIR=terminator_repo

BR=${1:-master}
PREFIX=${2:-/usr/local}
export PREFIX

#first install
if [ ! -d "$SETUP_DIR" ]; then
    #git clone git@github.com:gnome-terminator/terminator.git "$SETUP_DIR"
    git clone https://github.com/gnome-terminator/terminator.git "$SETUP_DIR"

    sudo apt install -y  python3-gi python3-gi-cairo python3-psutil python3-configobj \
                         gir1.2-keybinder-3.0 gir1.2-vte-2.91 gettext intltool dbus-x11 intltool
fi

pgrep -fa bin/terminator
sudo pkill -9 -f bin/terminator || echo "Can't close terminator"

cd "$SETUP_DIR"
if [[ ! -f install-files.txt ]]; then
    sudo python3 setup.py uninstall --manifest=install-files.txt
fi

git fetch -pt
git co "$BR"
git pull --rebase

sudo find "$PREFIX" -type f -name "terminator*"  --delete || echo "Can't remove terminator"
sudo find "$PREFIX" -type f -name "remotinator*" --delete || echo "Can't remove renominator"

sudo python3 setup.py build
sudo python3 setup.py install --single-version-externally-managed --record=install-files.txt
#sudo python3 setup.py install --single-version-externally-managed --record=install-files.txt --prefix=$PREFIX

sudo gtk-update-icon-cache -q -f "${PREFIX}/share/icons/hicolor"

echo 'Installed'

